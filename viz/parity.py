...
def plot_parity(traj, traj_nn, train_traj=None, title=None, save=None):
    ...
    if train_traj:
        ...
        f_dft_t = np.take(f_dft, train_list, axis=0)
        f_dft_v = np.take(f_dft, valid_list, axis=0)
        f_nn_t = np.take(f_nn, train_list, axis=0)
        f_nn_v = np.take(f_nn, valid_list, axis=0)
        ...
        x_text2 = np.mean(xx_e)-0.4*(np.max(np.concatenate((e_dft,e_nn)))-np.min(np.concatenate((e_dft,e_nn))))
        ...
        y_text2_t = np.mean(xx_e)+0.35*(np.max(np.concatenate((e_dft,e_nn)))-np.min(np.concatenate((e_dft,e_nn))))
        y_text2_v = np.mean(xx_e)+0.3*(np.max(np.concatenate((e_dft,e_nn)))-np.min(np.concatenate((e_dft,e_nn))))
        
        plt.figure(figsize=(10,7.5))
        if title:
            plt.title(title)
        plt.subplot(1, 2, 1)
        plt.plot(e_dft_t,e_nn_t,'o', color='tab:blue', alpha=0.5, label='Train')
        plt.plot(e_dft_v,e_nn_v,'o', color='tab:orange', alpha=0.5, label='Validation')
        plt.legend()
        ...
        plt.text(x_text2, y_text2_t, '$N_{Train}$ = %d'%(len(train_list)))
        plt.text(x_text2, y_text2_v, '$N_{Valid}$ = %d'%(len(valid_list)))
        ...
        plt.subplot(1, 2, 2)
        plt.plot(f_dft_t,f_nn_t,'o', color='tab:blue', alpha=0.5, label='Train')
        plt.plot(f_dft_v,f_nn_v,'o', color='tab:orange', alpha=0.5, label='Validation')
        plt.legend()
...
if __name__=="__main__":
    ...
    if args.t:
        ...
        plot_parity(traj, traj_nn, title = "$Al_{2}O_{3}$ + Hx + $Pt_{8}$ + $C_{3}H_{6}$ Perturb+MD+Opt Test")
