    ...
    for line in lines:
        ...
        if line_split[0].isdigit():
            ...
            if "stress" in line: 
                ...
                ind_stress_str_end = line[ind_stress_start + ind_stress_str_start + 1:].index('"')
                stress = [float(i) for i in line[ind_stress_str_start+ind_stress_start+1:ind_stress_start+ind_stress_str_end].split(" ")]
                
                dict_energies["stress"].append(np.linalg.norm(stress)) #np.array([np.linalg.norm(np.array(i)) for i in stress])
            ...
            for file in files:
                if file.endswith(".xyz"):
                    ...
                    if not os.path.exists(root + "/" + file[:-4] + ".csv"):
                        ...
                        iqr_range = 1.5
                        energy_outliers = list((energies < energy_25 - iqr_range * iqr_energy) | (energies > energy_75 + iqr_range * iqr_energy))
                        if ("bh" in file):
                            print("bh outlier finding")
                            force_outliers = list((forces < forces_25 - iqr_range * iqr_forces) | (forces > forces_75 + iqr_range * iqr_forces)) | (forces > 3) | (forces < -3)
                            ...
                            force_outliers = list((forces < forces_25 - iqr_range * iqr_forces) | (forces > forces_75 + iqr_range * iqr_forces)) 
